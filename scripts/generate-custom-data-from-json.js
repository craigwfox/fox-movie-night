...
function usage() {
	console.log(
		'Usage: node scripts/generate-custom-data-from-json.js [tokens.json path] [out path]'
	)
	console.log('')
...
function main() {
	...
	if (!fs.existsSync(tokensPath)) {
		...
		console.error(
			'If your tokens are in a different location, pass the path as the first argument.'
		)
		usage()
		process.exit(2)
	}
	...
	if (typeof tokens !== 'object' || Array.isArray(tokens) || tokens === null) {
		console.error(
			'Expected tokens.json to contain a top-level object of key/value pairs.'
		)
		process.exit(4)
	}
	...
	console.log(
		`Wrote CSS custom data for ${Object.keys(tokens).length} tokens to:`
	)
	console.log(`  ${outPath}`)
	console.log('')
	console.log(
		'To enable in VS Code workspace settings add/update .vscode/settings.json with:'
	)
	console.log('  {')
	...
	console.log('')
	console.log(
		'Reload the VS Code window (Developer: Reload Window) to pick up the new custom data.'
	)
}
